...
class UserProfileView: UIView, UserProfileViewProtocol {
    ...
    var collectionViewDragDelegate: CollectionViewDragDelegateProtocol
    var collectionViewDelegate: CollectionViewDelegateProtocol
    var collectionViewDataSource: CollectionViewDataSourceProtocol
    var collectionViewDropDelegate: CollectionViewDropDelegateProtocol
    ...
    init() {
        ...
        self.collectionViewDragDelegate = CollectionViewDragDelegate()
        self.collectionViewDropDelegate = CollectionViewDropDelegate()
    ...
    func buttonAlphaFalse() {
        button.alpha = 0
    }
    
    func buttonAlphaTrue() {
        button.alpha = 1
    }
}
...
private extension UserProfileView {
    ...
    func setupCollectionView() {
        ...
        collectionView.dragDelegate = collectionViewDragDelegate
        collectionViewDragDelegate.userProfileView = self
        collectionView.dropDelegate = collectionViewDropDelegate
        collectionViewDropDelegate.userProfileView = self
    ...
    func setupFlowLayout() {
        ...
        layout.minimumLineSpacing = 10 // мин.расстояние между ячейками по вертикали
        layout.minimumInteritemSpacing = 10 // мин.расстояние между ячейками по горизонтали
        layout.headerReferenceSize = .init(width: self.frame.size.width, height: 60) // размер заголовка коллекции
    }
...
protocol UserProfileViewProtocol: UIView {
    ...
    var collectionViewDragDelegate: CollectionViewDragDelegateProtocol { get set }
}
